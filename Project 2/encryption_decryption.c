...
{ 
    ...
    switch(a)
    {
       ...
       char plain_text[20];
		char encryp[20];
		int symkey;
		printf("Enter the plain_text text:		 ");
		scanf("%s",&plain_text);
		printf("Enter the symkey(<length(plain_text)):	 ");
		scanf("%d",&symkey);
		printf("Original Message:			 %s", plain_text);
		rail_encryption(plain_text, symkey);
		printf("\n");
		printf("Enter the cipher text:			 ");
		scanf("%s",&encryp);
		rail_decryption(encryp,symkey);
       break;
...
void rail_encryption(char plain_text[], int symkey){
int msgLen = strlen(plain_text), i, j, y = -1, row = 0, coloumn = 0;
char railMatrix[symkey][msgLen];//MATRIX 
for(i = 0; i < symkey; ++i)
for(j = 0; j < msgLen; ++j)
railMatrix[i][j] = '*'; //USED FOR DISTINGUISH FILLED FROM BLANK ONES
for(i = 0; i < msgLen; ++i){
railMatrix[row][coloumn++] = plain_text[i];//INSERTING THE PLAIN TEXT
if(row == 0 || row == symkey-1)
y= y * (-1);//USED FOR DIRECTION CHANGING
row = row + y;
}
printf("\nEncrypted Message:			 ");
...
for(i = 0; i < symkey; ++i)
for(j = 0; j < msgLen; ++j)
if(railMatrix[i][j] != '*')//USED FOR DISTINGUISHING THE TEXT AND BLANK
printf("%c", railMatrix[i][j]);
}

void rail_decryption(char cipher_text[], int symkey){
int meslen = strlen(cipher_text), i, j, y = -1, row = 0, coloumn = 0, m = 0;
char railMatrix[symkey][meslen];

for(i = 0; i < symkey; ++i)
for(j = 0; j < meslen; ++j)
railMatrix[i][j] = '@';//USED TO DISTINGUISH FILLED AND BLANK
for(i = 0; i < meslen; ++i){
railMatrix[row][coloumn++] = '#';//USED FOR MARKING THE PLACES WHERE TEXT WILL ENTER
if(row == 0 || row == symkey-1)
y= y * (-1);
row = row + y;
}
...
for(j = 0; j < meslen; ++j)
if(railMatrix[i][j] == '#')
railMatrix[i][j] = cipher_text[m++];//FILLING THE CIPHER TEXT
row = coloumn = 0;
y = -1;
...
for(i = 0; i < meslen; ++i){
printf("%c", railMatrix[row][coloumn++]);//READING THE MATRIX FOR GETTING PLAIN TEXT
if(row == 0 || row == symkey-1)
y= y * (-1);
row = row + y;
}
